...
export const Hero = () => {
  ...
  const orb1 = useParallax<HTMLDivElement>(0.35);
  const orb2 = useParallax<HTMLDivElement>(-0.3);
  const contentRef = useParallax<HTMLDivElement>(-0.06);

  return (
    <section className="relative min-h-screen flex items-center pt-28 pb-16 overflow-hidden bg-hero grain">
      <img ref={bgRef} src={heroBg} alt="" className="absolute inset-0 w-full h-[120%] object-cover opacity-40 mix-blend-screen animate-[float_18s_ease-in-out_infinite]" width={1920} height={1080} />
      <div ref={gridRef} className="absolute inset-0 grid-bg" />
      <div className="absolute inset-0 light-streak pointer-events-none" />
      <div ref={orb1} className="absolute -top-40 -left-40 w-[500px] h-[500px] rounded-full bg-secondary/20 blur-[120px] animate-pulse-glow" />
      <div ref={orb2} className="absolute -bottom-40 -right-40 w-[600px] h-[600px] rounded-full bg-accent/15 blur-[140px] animate-pulse-glow [animation-delay:1.5s]" />
      ...
      <div ref={contentRef} className="container relative z-10">
        <div className="max-w-5xl">
          <div className="inline-flex items-center gap-2 glass rounded-full px-4 py-2 mb-8 animate-fade-in">
            ...
            <span className="text-xs uppercase tracking-widest text-muted-foreground">Organised by The Sentry · Not for everyone</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-12 animate-fade-in-up [animation-delay:280ms]">
            <div className="relative animate-breathe">
              <span className="absolute inset-0 rounded-xl bg-brand blur-xl opacity-70 animate-pulse-glow pointer-events-none" />
          ...
          <div className="animate-fade-in-up [animation-delay:360ms]">
            <p className="text-xs uppercase tracking-widest text-accent mb-3 animate-flicker">⚡ Doors open in · Spots going fast</p>
            <Countdown />
          </div>
        </div>
      </div>
