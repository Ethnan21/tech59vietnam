...
import audienceEnterpriseAsset from "@/assets/audience-enterprise-new.jpg.asset.json";
const audienceEnterprise = audienceEnterpriseAsset.url;
    ...
    <section id="audience" className="py-24 relative">
      <div className="container">
        ...
        <div key={cur.key} className="relative glass-strong rounded-3xl p-6 sm:p-8 md:p-14 min-h-[260px] sm:min-h-[280px] md:min-h-[320px] animate-fade-in overflow-hidden grain max-w-full">
          <img src={cur.image} alt="" aria-hidden="true" loading="lazy" className={`absolute inset-0 w-full h-full object-cover opacity-50 pointer-events-none ${cur.key === 'investors' ? 'object-[center_70%]' : cur.key === 'ent' ? 'object-[center_bottom]' : ''}`} />
